...
const BankGallery = () => {
  ...
  const messages = [
    {
      heading: "Unlock Your Potential with Our Banking Video Series",
      paragraph: "Welcome to the Bank Video Gallery. These carefully curated videos are designed to help you understand the banking sector and prepare for your career in finance."
    },
    {
      heading: "Learn from Industry Experts and Enhance Your Knowledge",
      paragraph: "Our experienced instructors will guide you through key banking concepts and strategies. Browse the videos below to access insightful lessons and resources!"
    },
    {
      heading: "Join the Journey to Become a Banking Professional",
      paragraph: "Access valuable insights and materials that can propel your career in the banking industry. Start learning today and pave your way to success!"
    }
  ];
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const bankCategoryId = '6700d522c5e7f3f0191fcfb1'; // Replace with your actual Bank category ID
        const response = await axios.get(`https://adhikari-server.vercel.app/api/videos/category/${bankCategoryId}`);
        setVideos(response.data);
        setFilteredVideos(response.data); // Initialize filtered videos
      } catch (error) {
        console.error('Error fetching bank videos:', error);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between mb-3">
        <h2 className="text-3xl font-bold">Bank Video</h2>
          ...
          <div className="video-card bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 flex flex-col items-center max-w-xs" key={video._id}>
            ...
            <div className="p-1 flex flex-col items-center text-center rounded-lg"> {/* Reduced padding */}
              <h3 className="text-xl font-bold uppercase text-gray-800 font-sans flex items-center gap-1"> {/* Reduced gap between emoji and name */}
                🏦 {video.name} {/* Bank emoji for name */}
              </h3>
